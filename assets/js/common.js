jQuery(function ($) {
	...
	window.ScrollEffects = function () {
		...
		if (isMobile()) {
			...
			$('.smooth-scroll main, .has-parallax, nav, .anonymous-slider-wrapper:not(.content-slider), .showcase-lists .anonymous-sync-slider, .next-project-image-wrapper, .slider-fixed-content').css({ 'height': winHeight });
		...
		if ($('#anonymous-page-content').hasClass("light-content")) {
			if ($('#project-nav').hasClass("change-header")) {
				imagesLoaded('body', function () {
					setTimeout(function () {
						var pageNav = document.querySelector('#project-nav.change-header');
						if (pageNav) {
							var pageContent = document.querySelector('#anonymous-page-content');
							var nextPageCaptionParallax = gsap.to('.page-nav-caption', {
								scrollTrigger: {
									trigger: pageNav,
									start: "top 8%",
		...
		if ($('#anonymous-page-content').hasClass("dark-content")) {
			if ($('#project-nav').hasClass("change-header")) {
				imagesLoaded('body', function () {
					setTimeout(function () {
	...
	window.FirstLoad = function () {
		...
		if ($("#anonymous-page-content").hasClass("light-content")) {
			...
			gsap.to("main", { duration: 0.5, backgroundColor: document.querySelector("#anonymous-page-content").getAttribute("data-bgcolor"), ease: Power2.easeInOut });
		...
		$('a.ajax-link').on('click', function () {
			...
			gsap.to("nav", { duration: 0.3, backgroundColor: document.querySelector("#anonymous-page-content").getAttribute("data-bgcolor") });
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#itemsWrapper").hasClass("webgl-fitthumbs")) {
				...
				$('#itemsWrapperLinks .trigger-item-link, #itemsWrapperLinks .trigger-item-link-secondary').on('click', function () {
					...
					if (!$('#anonymous-page-content').hasClass("light-content")) {
						...
						if (!$('.portfolio').hasClass("portfolio-shortcode")) {
							if (!parent_item.hasClass("change-header")) {
								$('#anonymous-page-content').delay(1200).queue(function (next) {
					...
					$('.anonymous-slider .trigger-item').each(function () {
		...
		if (!$("body").hasClass("disable-ajaxload")) {
			...
			if ($("#itemsWrapper").hasClass("no-fitthumbs")) {
				...
				$('#itemsWrapperLinks .trigger-item-link').on('click', function () {
					...
					const fadeOutTargets = $('#hero, #show-filters, .anonymous-slider, .item, .item-caption-wrapper, #page-nav, footer, .fadeout-element');
					const ball = $('#ball');
	...
	window.Sliders = function () {
		...
		if ($('.content-slider').length > 0) {
			...
			$(".slider-button-prev").mouseenter(function (e) {
				if ($(this).parents(".anonymous-slider-wrapper").hasClass("light-cursor")) {
					$("body").addClass("drag-cursor-white");
					gsap.to('#ball', { duration: 0.2, borderWidth: '2px', scale: 1, borderColor: '#fff' });
				} else if ($(this).parents(".anonymous-slider-wrapper").hasClass("dark-cursor")) {
			...
			$('.content-slider.looped-carousel .anonymous-slider, .content-slider.small-looped-carousel .anonymous-slider').on('mouseenter mousemove', function () {
	...
	window.Lightbox = function () {
		...
		function showDetails(item) {
			...
			$('body').prepend(`<div class="anonymous-img-popup">
				<div class="anonymous-img-popup-bg-close"></div>				
				<div class="anonymous-img-popup-viewport">
					<div class="anonymous-img-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<img />
					<div class="anonymous-img-popup-close link"></div>						
				</div>				
				<div class="anonymous-img-popup-prev link"></div>
				<div class="anonymous-img-popup-next link"></div></div>`);

			const details = document.querySelector('.anonymous-img-popup');
			const detailsBgClose = document.querySelector('.anonymous-img-popup-bg-close');
			const detailsClose = document.querySelector('.anonymous-img-popup-close');
			const detailsPreloader = document.querySelector('.anonymous-img-popup-preloader');
			const detailImage = document.querySelector('.anonymous-img-popup img');
			const detailPrev = document.querySelector('.anonymous-img-popup-prev');
			const detailNext = document.querySelector('.anonymous-img-popup-next');
		...
		function showVideoDetails(event, item) {
			...
			$('body').prepend(`<div class="anonymous-video-popup">
				<div class="anonymous-video-popup-bg-close"></div>				
				<div class="anonymous-video-popup-viewport">
					<div class="anonymous-video-popup-preloader"><div></div><div></div><div></div><div></div></div>					
					<iframe class="anonymous-video-popup-iframe" frameborder="0" allow="autoplay"></iframe>
					<div class="anonymous-video-popup-close link"></div>						
				</div>				
				<div class="anonymous-img-video-prev link"></div>
				<div class="anonymous-img-video-next link"></div></div>`);

			const details = document.querySelector('.anonymous-video-popup');
			const detailsBgClose = document.querySelector('.anonymous-video-popup-bg-close');
			const detailsClose = document.querySelector('.anonymous-video-popup-close');
			const detailsPreloader = document.querySelector('.anonymous-video-popup-preloader');
			const detailIframe = document.querySelector('.anonymous-video-popup iframe');
	...
	window.Core = function () {
		...
		if (!isMobile() && !$('body').hasClass("disable-cursor")) {
			...
			$(".anonymous-button .sticky.left").mouseenter(function (e) {
				var rcBounds = $(this)[0].getBoundingClientRect();
			...
			$(".anonymous-button .sticky.right").mouseenter(function (e) {
				var rcBounds = $(this)[0].getBoundingClientRect();
			...
			$(".anonymous-button .parallax-wrap.icon-wrap").mouseenter(function (e) {
			...
			$(".anonymous-button .sticky").mouseenter(function (e) {
				if ($('#anonymous-page-content').hasClass("light-content")) {
			...
			$(".anonymous-button .parallax-wrap").mouseenter(function (e) {
				if ($('#anonymous-page-content').hasClass("light-content")) {
			...
			//Blog Hover Effects			
			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .anonymous-sidebar-widget a").mouseenter(function (e) {
				gsap.to('#ball', { duration: 0.2, borderWidth: '1px', scale: 1, opacity: 0 });
			});

			$("#blog-page-nav .page-numbers li a, .post-page-numbers, #post-content a, #post-form a, #post-comments a, .wp-block-search__button, .anonymous-sidebar-widget a").mouseleave(function (e) {
				gsap.to('#ball', { duration: 0.3, borderWidth: '4px', scale: 0.5, opacity: 1 });
			});
		}
