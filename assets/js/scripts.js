jQuery(function ($) {
		...
		function initOnFirstLoad() {
			...
			imagesLoaded('body', function () {
				...
				setTimeout(function () {
					...
					gsap.set($(".showcase-gallery .anonymous-slider .slide-inner"), { opacity: 0 });
					gsap.to($(".showcase-gallery .anonymous-slider .anonymous-slide .slide-inner"), { duration: 2, opacity: 1, delay: 0.3, ease: Power4.easeOut });

					var gallerySlideClasses = [".anonymous-slide-prev-two", ".anonymous-slide-prev", ".anonymous-slide-active", ".anonymous-slide-next", ".anonymous-slide-next-two"];

					gallerySlideClasses.forEach(function (gallerySlideClass, index) {
						var gallerySlide = $(".showcase-gallery .anonymous-slider " + gallerySlideClass + " .slide-inner");
					...
					gsap.set($("#filters-wrapper, .anonymous-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), { opacity: 0 });
					gsap.to($("#filters-wrapper, .anonymous-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), { duration: 0.4, opacity: 1, delay: 0.8, ease: Power3.easeOut });
	...
	function ShowcasePortfolio() {
		...
		if ($('.showcase-portfolio').length > 0) {
			...
			function filter() {
				var state = Flip.getState('.anonymous-item');
				var projects = document.querySelectorAll('.anonymous-item');
				var startHeight = gsap.getProperty(".showcase-portfolio", "height");

				var filters = document.querySelectorAll('.filter-option.is_active');
				var parallaxItems = document.querySelectorAll('.showcase-portfolio .anonymous-item.vertical-parallax .slide-inner');
	...
	function ShowcaseGallery() {
		...
		if ($('.showcase-gallery').length > 0) {
			...
			slider = new ClapatSlider('.showcase-gallery', {
				...
				on: {
					init: function (slide) {
						...
						if ($('body').hasClass("show-loader")) {
							...
							imagesLoaded('body', function () {
								...
								gsap.to($(".showcase-gallery .anonymous-slider .anonymous-slide .slide-inner"), { duration: 0.7, opacity: 1, delay: 0.4, ease: Power2.easeOut });
								...
								gsap.to($("#filters-wrapper, .anonymous-pagination, .cp-button-prev, .cp-button-next, .progress-info, footer .link-text"), { duration: 0.5, opacity: 1, delay: 0.4, ease: Power2.easeOut });
					...
					slideLeaveViewport: function (slide) {
						gsap.set($('.anonymous-slider div:not(.anonymous-slide-visible) .slide-events'), { x: "" });
					},
			...
			const showGrid = () => {
				...
				gsap.to($("footer .link-text, .anonymous-pagination, .progress-info, #filters-wrapper"), { duration: 0.3, opacity: 0, y: 30, stagger: -0.05, ease: Power2.easeInOut });
				...
				gsap.set(currentImage.closest(".anonymous-slide"), { zIndex: 0 });
				...
				gsap.to(".anonymous-slider .anonymous-slide .trigger-item", { duration: 1, opacity: 0, scale: 0.7, ease: Power2.easeOut });
			...
			const hideGrid = () => {
				...
				gsap.to(".anonymous-slider .anonymous-slide .trigger-item", { duration: 0.5, opacity: 1, scale: 1, delay: 0.2, ease: 'power3.inOut' });
				...
				Flip.from(flipstate1, {
					...
					onComplete: function () {
						...
						const triggeredItem = document.querySelector('.anonymous-slide.triggered-item');
						if (triggeredItem != null) {
							triggeredItem.classList.remove('triggered-item');
						}

						const clapatSlides = document.querySelectorAll('.anonymous-slide');
				...
				gsap.to($("footer .link-text, .anonymous-pagination, .progress-info, #filters-wrapper"), { duration: 0.3, opacity: 1, y: 0, stagger: 0.05, delay: 0.4, ease: Power2.easeInOut });
				gsap.to('#ball', { duration: 0.2, borderWidth: '4px', scale: 0.5, borderColor: '#999999', backgroundColor: 'transparent' });
				gsap.to('#ball-loader', { duration: 0.2, borderWidth: '4px', top: 0, left: 0 });
				$("#ball").removeClass("with-blur");
				$('#ball p').remove();
			...
			const getDOMElements = () => {
				...
				const items = [];
				[...document.querySelectorAll('.anonymous-slide')].forEach(item => {
					items.push(new Item(item));
				});

				// Cloned items
				const itemsCloned = [];
				[...document.querySelectorAll('.anonymous-slide-clone')].forEach(item => {
				...
				return {
					items: items,
					grid: gridItems,
					currentItem: new Item(document.querySelector('.anonymous-slide.triggered-item')),
			...
			const initEvents = () => {
				...
				items.forEach((triggerItem) => {
					...
					triggerItem.addEventListener('click', (event) => {
						...
						if ($('.showcase-gallery').length > 0) {
							...
							event.currentTarget.closest('.anonymous-slide').classList.add('triggered-item');
							showGrid();
						}
					});
				...
				function moveThumbGrid(direction = "next") {
					...
					if (document.body.classList.contains("grid-open")) {
						...
						// Get all the slides
						let slides = document.querySelectorAll('.anonymous-slide');
						...
						$('.anonymous-slide').removeClass('triggered-item');
			...
			if (!isMobile()) {
				...
				if ($('.showcase-gallery').hasClass("tilt-gallery")) {
					...
					function callTiltSlider(e) {
						moveItSlider(e, '.anonymous-slider-viewport', 30);
					}
				...
				var dragWrapper = $('.anonymous-slider');
				...
				$('.anonymous-slider').on('mouseup touchend', function () {
				...
				$(".showcase-gallery.preview-mode-enabled .anonymous-slide .slide-inner").on('mouseenter', function () {
